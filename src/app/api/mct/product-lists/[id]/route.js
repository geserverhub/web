...
export async function GET(req, { params }) {
  ...
  try {
    ...
    const productList = await prisma.mGroupOrder.findUnique({
      where: { id },
      include: { items: true, client: { select: { id: true, name: true, slug: true } } },
    });
    if (!productList) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (scope.isClient && productList.clientId !== scope.clientId) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
    return NextResponse.json({ productList });
  } catch (error) {
    return NextResponse.json({ error: error?.message || "Failed to fetch product list" }, { status: 500 });
  }
}
...
export async function PATCH(req, { params }) {
  ...
  try {
    ...
    const productList = await prisma.mGroupOrder.update({
      where: { id },
      data: {
        status: nextStatus,
    ...
    return NextResponse.json({ error: error?.message || "Failed to update product list" }, { status: 500 });
  }
}
