...
export async function POST(req, { params }) {
  ...
  try {
    ...
    const order = await prisma.mGroupOrder.findUnique({ where: { id }, select: { id: true, clientId: true, status: true } });
    ...
    const updated = await prisma.mGroupOrder.update({
      ...
      data: {
        ...
        status: order.status === "PENDING_PAYMENT" ? "CONFIRMING" : order.status,
        paidAt: order.status === "PENDING_PAYMENT" ? new Date() : undefined,
      },
      include: { items: true, client: { select: { id: true, name: true, slug: true } } },
    });

    return NextResponse.json({ order: updated, paymentSlipUrl });
  } catch (error) {
