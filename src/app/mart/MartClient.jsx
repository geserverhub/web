...
const symOf = (cur) => CURRENCIES.find(c => c.value === cur)?.symbol || "฿";
...
export default function MartClient({ session }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [p, c] = await Promise.all([
        apiFetch("/api/mart/products?active=0"),
        apiFetch("/api/mart/categories"),
      ]);
  ...
  async function saveProduct() {
    if (!form.name || !form.category || !form.price) {
      showToast("กรุณากรอก ชื่อสินค้า / หมวด / ราคา", false);
      return;
    }
    ...
    try {
      ...
      if (editId) {
        await apiFetch(`/api/mart/products/${editId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
      } else {
        await apiFetch("/api/mart/products", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
  ...
  async function deleteProduct(p) {
    ...
    try {
      await apiFetch(`/api/mart/products/${p.id}`, { method: "DELETE" });
  ...
  async function toggleActive(p) {
    try {
      await apiFetch(`/api/mart/products/${p.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function addCategory() {
    ...
    try {
      const d = await apiFetch("/api/mart/categories", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function removeCategory(name) {
    ...
    try {
      const d = await apiFetch(`/api/mart/categories?name=${encodeURIComponent(name)}`, { method: "DELETE" });
      const next = Array.isArray(d.categories) ? d.categories : [];
      setCategories(next);
  ...
  async function importFromExcel(file) {
    ...
    try {
      const fd = new FormData();
      fd.append("file", file);
      const d = await apiFetch("/api/mart/products/import", { method: "POST", body: fd });
      setImportResult(d);
      showToast(d.message || `นำเข้า ${d.imported} รายการ`);
      await loadAll();
    } catch (err) {
      setImportResult({ ok: false, message: err.message });
      showToast(err.message || "นำเข้าไม่สำเร็จ", false);
    } finally {
      setImporting(false);
  ...
  function downloadTemplate() {
    const header = ["sku", "category", "name", "nameEn", "nameZh", "price", "priceWholesale", "unit", "minOrder", "minWholesale", "stock", "desc", "img", "active", "promotion", "promotionPrice"];
    const example = ["MART00001", "snack", "ขนมอบกรอบ", "Crispy Snack", "", "50", "40", "ชิ้น", "1", "10", "100", "ขนมอบกรอบรสชาติดี", "", "true", "ลด 10%", "45"];
    const csv = [header, example].map(r => r.join(",")).join("\n");
    const blob = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "mart-products-template.csv";
    a.click();
  }
    ...
    <div style={S.bg}>
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button style={{ ...S.btn("#0f2b1a", "#4ade80"), border: "1px solid #166534" }}
            onClick={() => excelRef.current?.click()} disabled={importing}>
            {importing ? "⏳ กำลังนำเข้า..." : "📥 นำเข้า Excel"}
          </button>
          <button style={{ ...S.btn("#1a1d27", "#8b8fa8"), border: "1px solid #2a2d3a", fontSize: 12 }}
      ...
      {/* Import result panel */}
      {importResult && (
        <div style={{ background: importResult.ok ? "#052e16" : "#450a0a", border: `1px solid ${importResult.ok ? "#166534" : "#7f1d1d"}`, margin: "12px 24px 0", borderRadius: 10, padding: "14px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontWeight: 700, color: importResult.ok ? "#4ade80" : "#f87171", fontSize: 14, marginBottom: 6 }}>
                {importResult.ok ? "✅" : "❌"} {importResult.message}
              </div>
              {importResult.ok && (
                <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#8b8fa8" }}>
                  <span style={{ color: "#4ade80" }}>นำเข้าสำเร็จ: {importResult.imported} รายการ</span>
                  {importResult.skipped > 0 && <span style={{ color: "#fbbf24" }}>ข้าม: {importResult.skipped} รายการ</span>}
                </div>
              )}
              {importResult.errors?.length > 0 && (
                <div style={{ marginTop: 8, maxHeight: 80, overflowY: "auto" }}>
                  {importResult.errors.map((e, i) => (
                    <div key={i} style={{ fontSize: 11, color: "#f87171", marginBottom: 2 }}>• {e}</div>
                  ))}
                </div>
              )}
            </div>
            <button style={{ background: "none", border: "none", color: "#8b8fa8", cursor: "pointer", fontSize: 18 }} onClick={() => setImportResult(null)}>✕</button>
          </div>
        </div>
      )}
                ...
                <tbody>
                  {filteredProducts.map((p, idx) => (
                    <tr key={p.id} style={{ background: idx % 2 === 0 ? "transparent" : "#0f111633", opacity: p.active ? 1 : 0.65 }}>
                      ...
                      <td style={{ ...S.td, color: "#4ade80", fontWeight: 700 }}>{symOf(p.currency)}{Number(p.price).toLocaleString()}</td>
                      <td style={{ ...S.td, color: "#67e8f9", fontWeight: 700 }}>{p.priceWholesale ? `${symOf(p.currency)}${Number(p.priceWholesale).toLocaleString()}` : "—"}</td>
                      <td style={S.td}>
                        {p.promotion ? (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ ...S.card, width: "100%", maxWidth: 920, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 10 }}>
              <div>
                <label style={S.label}>SKU <span style={{ color: "#4a5070", fontWeight: 400, textTransform: "none", fontSize: 10 }}>(ว่าง = รันอัตโนมัติ)</span></label>
                <div style={{ display: "flex", gap: 6 }}>
                  <input style={{ ...S.input, flex: 1 }} placeholder={`SKU${new Date().toISOString().slice(2,4)}${String(new Date().getMonth()+1).padStart(2,"0")}${String(new Date().getDate()).padStart(2,"0")}0001`}
                    value={form.sku} onChange={(e) => setForm((f) => ({ ...f, sku: e.target.value.toUpperCase() }))} />
                  <button type="button" title="ล้าง SKU ให้รันอัตโนมัติ"
                    style={{ ...S.btn("#1e2130", "#8b8fa8"), border: "1px solid #2a2d3a", padding: "8px 10px", fontSize: 11, whiteSpace: "nowrap" }}
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {CURRENCIES.map(c => (
                    <button key={c.value} type="button"
                      ...
                      style={{ flex: 1, padding: "8px 4px", borderRadius: 8, border: form.currency === c.value ? "2px solid #f472b6" : "1px solid #2a2d3a", background: form.currency === c.value ? "#2d0f3a" : "#1e2130", color: form.currency === c.value ? "#f472b6" : "#8b8fa8", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                      {c.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 0 }}>
                  <span style={{ background: "#2a2d3a", border: "1px solid #2a2d3a", borderRight: "none", borderRadius: "8px 0 0 8px", padding: "8px 12px", color: "#f472b6", fontWeight: 800, fontSize: 15 }}>{symOf(form.currency)}</span>
                  <input type="number" style={{ ...S.input, borderRadius: "0 8px 8px 0", borderLeft: "none" }} value={form.price} onChange={(e) => setForm((f) => ({ ...f, price: e.target.value }))} />
                </div>
              </div>
              <div>
                <label style={S.label}>ราคาส่ง</label>
                <div style={{ display: "flex", gap: 0 }}>
                  <span style={{ background: "#2a2d3a", border: "1px solid #2a2d3a", borderRight: "none", borderRadius: "8px 0 0 8px", padding: "8px 12px", color: "#67e8f9", fontWeight: 800, fontSize: 15 }}>{symOf(form.currency)}</span>
                  <input type="number" style={{ ...S.input, borderRadius: "0 8px 8px 0", borderLeft: "none" }} value={form.priceWholesale} onChange={(e) => setForm((f) => ({ ...f, priceWholesale: e.target.value }))} />
                </div>
              </div>
              <div>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 12, alignItems: "center" }}>
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    <button style={S.btn("#1e2130", "#8b8fa8")} disabled={imgUploading} onClick={() => fileRef.current?.click()}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14 }}>
              <button style={S.btn("#1e2130", "#8b8fa8")} onClick={() => setModalOpen(false)}>ยกเลิก</button>
              <button style={S.btn("#2d0f3a", "#f472b6")} onClick={saveProduct} disabled={saving}>
