...
const pool = mysql.createPool({
  ...
  password: process.env.MYSQL_PASSWORD || 'EnergyAdmin2025',
  database: 'ksystem',
  connectionLimit: 10,
  timezone: '+00:00'
})
...
/**
 * Query energy database with parameterized statements
 */
export async function queryEnergy(sql: string, values?: any[]): Promise<any[]> {
...
/**
 * Get all devices from energy.devices table with real-time status from power_records
 * Status logic:
...
export async function getAllDevices(): Promise<any[]> {
  try {
    const devices = await queryEnergy(
      `SELECT
        d.deviceID,
        d.deviceName,
        d.ksaveID AS energyID,
        d.location,
        d.ipAddress,
        d.beforeMeterNo,
        d.metricsMeterNo,
    ...
    console.error('Error fetching devices from energy:', error)
    return []
  }
}
...
export async function getDeviceById(deviceID: number): Promise<any | null> {
  try {
    const devices = await queryEnergy(
      `SELECT deviceID, deviceName, ksaveID AS energyID, location, status, ipAddress, 
    ...
    console.error('Error fetching device from energy:', error)
    return null
  }
}

/**
 * Get device by energyID
 */
export async function getDeviceByEnergyId(energyID: string): Promise<any | null> {
  try {
    const devices = await queryEnergy(
      `SELECT deviceID, deviceName, ksaveID AS energyID, location, status, ipAddress, 
              beforeMeterNo, metricsMeterNo, created_at, updated_at
